...
/* Private functions ---------------------------------------------------------*/
static rt_tick_t timer = 0;
static void m_timer_start(void)
{
	timer = rt_tick_get();
	rt_kprintf("start timer: %dms\r\n",timer);
}
static void m_timer_stop(void)
{
	rt_tick_t now_time,print_time;
	now_time = rt_tick_get();
	print_time = now_time - timer;
	rt_kprintf("stop timer: %dms\r\n",now_time);
	rt_kprintf("timer: %dms\r\n",print_time);
}
...
/**
  ...
  * @retval None
  */
static void rsa_test(int argc, char *argv[])
//int rsa_test(void)
{
  ...
  if (status == RSA_SUCCESS)
  {
    ...
    {
      /* add application traintment in case of RSA sign is passed */
			rt_kprintf("add application traintment in case of RSA sign is passed\r\n");
    }
    else
    {
      /* add application traintment in case of RSA sign is failed */
			rt_kprintf("add application traintment in case of RSA sign is failed\r\n");
    }
		m_timer_start();
    ...
    if (status == SIGNATURE_VALID)
    {
      /* Signature has been validated */
			rt_kprintf("Signature has been validated\r\n");
  ...
  /* Infinite loop
   * Microcontroller's duties */
//  while (1)
//  {}

}
MSH_CMD_EXPORT(rsa_test, rsa sample);
/**
